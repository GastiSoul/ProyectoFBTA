...
namespace DemoP
{
    public partial class GestUsuario : System.Web.UI.Page
    {
        ...
        {
            // Verificar si el usuario tiene permisos de modificar
            bool userHasPermissions = Session["UserHasPermissions"] != null && (bool)Session["UserHasPermissions"];

            // Si el usuario no tiene permisos, deshabilitar la edición en el GridView
            if (!userHasPermissions)
            {
                GridView1.Columns[0].Visible = false; // Suponiendo que la primera columna es la de edición
            }
        }
