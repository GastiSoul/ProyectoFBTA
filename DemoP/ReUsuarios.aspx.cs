                ...
                using (SqlCommand command = new SqlCommand(query, coneccion))
                {
                    ...
                    if (Regex.IsMatch(matricula, @"^\d+$"))
                    {
                        try
                        {
                            ...
                            if (rowsAffected > 0)
                            {
                                Response.Redirect("MenuPrincipal.aspx?registro=exitoso");
                            }
                            else
                            {
                        ...
                        {
                            ClientScript.RegisterStartupScript(this.GetType(), "error", "alert('Error: El correo o ID ingresados ya existen');", true);
                        }
                    }
                    else
                    {
